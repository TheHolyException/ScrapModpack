Fant_Rotor_Large = class()
Fant_Rotor_Large.maxParentCount = 4
Fant_Rotor_Large.connectionInput = sm.interactable.connectionType.logic
Fant_Rotor_Large.TiltSpeed = 1
Fant_Rotor_Large.SliderSteps = 10
Fant_Rotor_Large.Force = 350
 
function Fant_Rotor_Large.server_onCreate( self )
...
function Fant_Rotor_Large.client_onCreate( self )
...
function Fant_Rotor_Large.server_onDestroy( self )	
	self.saved = { tilt = self.tilt, length = self.length, defaultTilt = self.defaultTilt }
	self.storage:save( self.saved )
end

function Fant_Rotor_Large.sv_getData( self )
	self.network:sendToClients( "cl_set_data", { tilt = self.tilt, length = self.length, defaultTilt = self.defaultTilt } )
end

function Fant_Rotor_Large.cl_set_data( self, data )
...
function Fant_Rotor_Large.getParentInputs( self )
	local down = false
	local up = false
	local fineTune = 0
...
function Fant_Rotor_Large.server_onFixedUpdate( self, dt )
...
function Fant_Rotor_Large.cl_set_tilt( self, data )
	self.animation_tilt = data.tilt
	self.animation_defaultTilt = data.defaultTilt
...
function Fant_Rotor_Large.client_onTinker( self, character, state )
	if state == true then
...
function Fant_Rotor_Large.cl_onSliderChange( self, sliderName, sliderPos )
...
function Fant_Rotor_Large.setSliderValue( self, sliderValue )
...
function Fant_Rotor_Large.client_onInteract( self, character, state )
	if state == true then
...
function Fant_Rotor_Large.cl_onSliderChange2( self, sliderName, sliderPos )
...
function Fant_Rotor_Large.setSliderValue2( self, sliderValue2 )
