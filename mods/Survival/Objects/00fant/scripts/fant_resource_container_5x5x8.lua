dofile("$SURVIVAL_DATA/Scripts/game/interactables/ResourceContainer.lua")

Fant_Resource_Container_5x5x8 = class( nil )
Fant_Resource_Container_5x5x8.ContainerSize = 20


local ModulesItems = {
	{ Uuid = obj_fant_upgradeModule_3, Range = 3 }
}


function Fant_Resource_Container_5x5x8.server_onCreate( self )
	self.sv = {}
	...
	local containerModule =  self.interactable:getContainer(1)
	if not containerModule then
		containerModule =  self.interactable:addContainer( 1, 1, 1 )
	end
	...
	for i, m in pairs( ModulesItems ) do
		if self.Module == m.Uuid then
			Range = m.Range
		end
	end
	
	local shapeSize = sm.item.getShapeSize( self.shape:getShapeUuid() ) * 0.125
	local size = sm.vec3.new( shapeSize.x + 0.875, shapeSize.y + 0.875, shapeSize.z + 0.875 ) * Range
...
function Fant_Resource_Container_5x5x8.server_onFixedUpdate( self )
...
function Fant_Resource_Container_5x5x8.trigger_onEnter( self, trigger, contents )
	--print(contents)
...
function Fant_Resource_Container_5x5x8.sv_e_receiveItem( self, params )
...
function Fant_Resource_Container_5x5x8.sv_n_takeHarvest( self, params )
	local container = self.interactable:getContainer( 0 )
	if container then
...
function Fant_Resource_Container_5x5x8.sv_rearrangeContents( self, removedSlotIndex )
	local container = self.interactable:getContainer( 0 )
	if container then
...
function Fant_Resource_Container_5x5x8.cl_n_updateRearrange( self, params )
	local container = self.interactable:getContainer( 0 )
	if container then
		for i = 1, self.ContainerSize do
			local slotItem = container:getItem( i - 1 )
...
function Fant_Resource_Container_5x5x8.client_onCreate( self )
	self:cl_init()
end

function Fant_Resource_Container_5x5x8.client_onRefresh( self )
	if self.cl then
...
function Fant_Resource_Container_5x5x8.client_onDestroy( self )
	for _, harvestItem in ipairs( self.cl.harvestItems ) do
		if harvestItem.effect then
			harvestItem.effect:stop()
		end
...
function Fant_Resource_Container_5x5x8.cl_init( self )
	...
	for i = 1, self.ContainerSize do
		...
		harvestItem.effect:setScale( sm.vec3.new( 0.25, 0.25 / 1.5, 0.25 ) )
...
function Fant_Resource_Container_5x5x8.cl_n_addPickupItem( self, params )
	if self.cl == nil then
		self.cl = {}
	end
...
function Fant_Resource_Container_5x5x8.client_onUpdate( self, dt )
...
function Fant_Resource_Container_5x5x8.client_canInteract( self )
	local container = self.interactable:getContainer( 0 )
...
function Fant_Resource_Container_5x5x8.client_onInteract( self, user, state )
	if state then
		local container = self.interactable:getContainer( 0 )
...
function Fant_Resource_Container_5x5x8.client_onTinker( self, character, state )
	if state == true then 
		local gui = sm.gui.createContainerGui( true )
		gui:setText( "UpperName", "Resource Modules" )
...
function Fant_Resource_Container_5x5x8.ModuleManager( self )
	local uuid = nil
	local Range = 1
	for i, m in pairs( ModulesItems ) do
...
function Fant_Resource_Container_5x5x8.server_canErase( self )
	local containerModule = self.shape:getInteractable():getContainer(1)

	if not containerModule:isEmpty() then
		return false
	end
	return true
end

function Fant_Resource_Container_5x5x8.client_canErase( self )
	local containerModule = self.shape:getInteractable():getContainer(1)

	if not containerModule:isEmpty() then
		sm.gui.displayAlertText( "Remove Module", 1.5 )
		return false
	end
	return true
end
...
function Fant_Resource_Container_5x5x8.cl_getRaycastSlot( self )
	local container = self.interactable:getContainer( 0 )
	...
	if success then
		...
		local slotIndex = math.min( math.max( xIndex + yIndex, 0 ), 19 )
...
function Fant_Resource_Container_5x5x8.calculateSlotItemOffset( self, slotIndex )
	local width = 4
	local height = 5
	local offsetCornerPosition = sm.vec3.new( -0.375, -0.5, 0 )
