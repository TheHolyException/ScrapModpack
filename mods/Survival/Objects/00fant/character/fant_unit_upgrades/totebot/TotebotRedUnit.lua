...
function TotebotRedUnit.server_onCreate( self )
	...
	if self.saved.stats == nil then
		self.saved.stats = { hp = 20, maxhp = 20 }
	end
	...
	-- 00Fant
	-- if self.saved.Buff_Character == nil then
		-- if math.random( 0, 100 ) > 75 then
			-- self.saved.Buff_Character = true
			
		-- else
			-- self.saved.Buff_Character = false
		-- end
		-- self.storage:save( self.saved )
	-- end
	
	if self.params and self.saved.Buff_Character == nil then
		if self.params.inWater then
			self.saved.Buff_Character = true
		else
			self.saved.Buff_Character = false
		end
	...
	if self.saved.Buff_Character then
		self.unit:getCharacter().movementSpeedFraction = 2.5
	end
...
function TotebotRedUnit.server_onRefresh( self )
	print( "-- TotebotGreenUnit refreshed --" )
end

function TotebotRedUnit.server_onDestroy( self )
	print( "-- TotebotGreenUnit terminated --" )
end

function TotebotRedUnit.server_onFixedUpdate( self, dt )
	-- 00Fant
	...
	self:sv_set_Buff_Character()
	
	ProcessTeslaDamage( self, false )
	FlamethrowerDamage( self, dt )
	
	if sm.game.getEnableAggro() then
		for i, player in pairs( sm.player.getAllPlayers() ) do
			if sm.vec3.length( player.character.worldPosition - self.unit.character.worldPosition ) < math.random( 3, 4 ) then
				self:Explode()
				self.unit:destroy()
				self.destroyed = true
			end
	...
	if updateCrushing( self ) then
		print("'TotebotGreenUnit' was crushed!", type( self.attacker ))
		self:sv_onDeath( sm.vec3.new( 0, 0, 0 ) )
		-- 00Fant
		if type( self.attacker ) == "Player" then				

			if self.saved.Buff_Character then
				sm.event.sendToPlayer( self.attacker, "sv_addRobotKill",  { attacker = self.attacker, typeName = "heavytotebot" } )
			else
				sm.event.sendToPlayer( self.attacker, "sv_addRobotKill",  { attacker = self.attacker, typeName = "totebot" } )
			end
		end
		-- 00Fant
	end
...
function TotebotRedUnit.server_onCharacterChangedColor( self, color )
...
function TotebotRedUnit.server_onUnitUpdate( self, dt )
...
function TotebotRedUnit.sv_e_worldEvent( self, params )
...
function TotebotRedUnit.server_onProjectile( self, hitPos, hitTime, hitVelocity, projectileName, attacker, damage )
	...
	if type( attacker ) == "Player" or type( attacker ) == "Shape" or teamOpponent then
		...
		-- 00Fant "attacker"
		self:sv_takeDamage( damage, impact, hitPos, attacker )
	end
end

function TotebotRedUnit.server_onMelee( self, hitPos, attacker, damage )
	if not sm.exists( self.unit ) or not sm.exists( attacker ) then
		return
	end
...
function TotebotRedUnit.server_onExplosion( self, center, destructionLevel )
	if not sm.exists( self.unit ) then
		return
	end
	local impact = 5
	if self.unit:getCharacter().worldPosition - center ~= sm.vec.new( 0, 0, 0 ) then
		impact = ( self.unit:getCharacter().worldPosition - center ):normalize() * 6
	end
	self:sv_takeDamage( self.saved.stats.maxhp * ( destructionLevel / 10 ), impact, self.unit:getCharacter().worldPosition )
end

function TotebotRedUnit.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal )
	...
	if damage > 0 then
		...
		self:sv_takeDamage( damage, collisionNormal, collisionPosition, self.attacker )
	end
...
function TotebotRedUnit.server_onCollisionCrush( self )
	if not sm.exists( self.unit ) then
		return
	end
...
function TotebotRedUnit.sv_updateCharacterTarget( self )
	if self.unit.character then
		sm.event.sendToCharacter( self.unit.character, "sv_n_updateTarget", { target = self.target } )
	end
end

function TotebotRedUnit.sv_addStagger( self, stagger )
...
function TotebotRedUnit.sv_takeDamage( self, damage, impact, hitPos, attacker )
	if self.saved.stats.hp > 0 then
		...
		if self.saved.stats.hp <= 0 then
			...
			if type( attacker ) == "Player" then				
				if self.saved.Buff_Character then
					sm.event.sendToPlayer( attacker, "sv_addRobotKill",  { attacker = attacker, typeName = "heavytotebot" } )
				else
					sm.event.sendToPlayer( attacker, "sv_addRobotKill",  { attacker = attacker, typeName = "totebot" } )
				end
			end
...
function TotebotRedUnit.sv_onDeath( self, impact )
...
function TotebotRedUnit.sv_spawnParts( self, impact )
	local character = self.unit:getCharacter()
	...
	if isSurvival then
		if self.Totebothammerhit ~= nil and self.Totebothammerhit then
			local params = { lootUid = obj_interactive_robotsynthhead, lootQuantity = 1, epic = false }
			sm.projectile.customProjectileAttack( params, "loot", 0, bodyPos, sm.vec3.new( 0, 0, 0 ), self.unit, bodyPos, bodyPos, 0 )
			
			local params = { lootUid = obj_resource_circuitboard, lootQuantity = math.floor(  math.random( 2, 3 ) ), epic = false }
			sm.projectile.customProjectileAttack( params, "loot", 0, bodyPos, sm.vec3.new( 0, 0, 0 ), self.unit, bodyPos, bodyPos, 0 )
		else
			...
			local ScrewsAndNutsAmount = math.random( 0, 2 )
			if ScrewsAndNutsAmount > 0 then
				if math.random( 0, 1 ) > 0.5 then
					local params = { lootUid = obj_decor_screw01, lootQuantity = ScrewsAndNutsAmount, epic = false }
					sm.projectile.customProjectileAttack( params, "loot", 0, bodyPos, sm.vec3.new( 0, 0, 0 ), self.unit, bodyPos, bodyPos, 0 )
				else
					local params = { lootUid = obj_decor_screw02, lootQuantity = ScrewsAndNutsAmount, epic = false }
					sm.projectile.customProjectileAttack( params, "loot", 0, bodyPos, sm.vec3.new( 0, 0, 0 ), self.unit, bodyPos, bodyPos, 0 )
				end
			end
		end
...
function TotebotRedUnit.sv_e_receiveTarget( self, params )
...
function TotebotRedUnit.sv_e_onEnterWater( self )
	ExtinguishFire( self )
end

function TotebotRedUnit.sv_e_onStayWater( self ) end

function TotebotRedUnit.sv_Totebothammerhit( self ) 
	--print( "sv_Totebothammerhit"  )
	self.Totebothammerhit = true
end



-- 00Fant
function TotebotRedUnit.sv_takeDamage2( self, data )
	self:sv_takeDamage( data.damage, sm.vec3.new( 0, 0, 1 ), self.unit:getCharacter().worldPosition, data.attacker )
end



function TotebotRedUnit.sv_set_Buff_Character( self )
	if self.unit:getCharacter() == nil then
		return
	end
	if self.saved.Buff_Character == nil then
		return
	end
	if not self.saved.Buff_Character then
		return
	end
	if self.Buff_Character_Init == nil then
		self.Buff_Character_Init = 20
	end
	if self.Buff_Character_Init >= 0 then
		self.Buff_Character_Init = self.Buff_Character_Init - 1
		if self.Buff_Character_Init < 0 then
			self.unit:sendCharacterEvent( "buff_character" )
			if isSurvival then
				self.unit.character:setColor( sm.color.new( 0.1, 0.1, 1.0, 1 ) )
			end
			local buff_mul = 2
			self.saved.stats = { hp = 100 * buff_mul, maxhp = 100 * buff_mul }
...
function TotebotRedUnit.Explode( self )
	...
	sm.physics.explode( self.unit.character.worldPosition, self.destructionLevel, self.destructionRadius, self.impulseRadius, self.impulseMagnitude, self.explosionEffectName )
