...
TotebotCharacterRed = class( Character )
...
function TotebotCharacterRed.client_onCreate( self )
	self.cl = {}
	self.cl.animations = {}
...
function TotebotCharacterRed.client_onDestroy( self )
	--print( "-- TotebotCharacter destroyed --" )
end

function TotebotCharacterRed.client_onRefresh( self )
	--print( "-- TotebotCharacter refreshed --" )
end

function TotebotCharacterRed.client_onGraphicsLoaded( self )
...
function TotebotCharacterRed.client_onGraphicsUnloaded( self )
...
function TotebotCharacterRed.cl_initGraphics( self )
...
function TotebotCharacterRed.cl_initAnimationSwitch( self )
...
function TotebotCharacterRed.client_onUpdate( self, deltaTime )
...
function TotebotCharacterRed.client_onEvent( self, event )
	if event == "buff_character" then
		self.character:addRenderable( "$SURVIVAL_DATA/Objects/00fant/character/fant_unit_upgrades/totebot/totebot_armor_1.rend" )
		self.character:addRenderable( "$SURVIVAL_DATA/Objects/00fant/character/fant_unit_upgrades/totebot/totebot_armor_2.rend" )
		--self.cl.effects.sparks:stop()
	end
	self:cl_handleEvent( event )
end

function TotebotCharacterRed.cl_handleEvent( self, event )
...
function TotebotCharacterRed.sv_n_updateTarget( self, params )
	self.network:sendToClients( "cl_n_updateTarget", params )
end

function TotebotCharacterRed.cl_n_updateTarget( self, params )
	self.cl.target = params.target
end

function TotebotCharacterRed.sv_e_unitDebugText( self, text )
	-- No sync cheat
