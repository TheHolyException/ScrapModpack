...
function HeavyHayBotUnit.server_onCreate( self )
	...
	-- 00Fant
	self.unit:getCharacter().movementSpeedFraction = 0.8
	if isSurvival then
		self.unit.character:setColor( sm.color.new( 0.2, 0.15, 0.1, 1 ) )
		SpawnStraw_dog( self )
	end
...
function HeavyHayBotUnit.server_onRefresh( self )
	print( "-- HaybotUnit refreshed --" )
end

function HeavyHayBotUnit.server_onDestroy( self )
	print( "-- HaybotUnit terminated --" )
end

function HeavyHayBotUnit.server_onFixedUpdate( self, dt )
	...
	if updateCrushing( self ) then
		print("'HaybotUnit' was crushed!", type( self.attacker ))
		self:sv_onDeath( sm.vec3.new( 0, 0, 0 ) )
		-- 00Fant
		if type( self.attacker ) == "Player" then				
			if self.saved.Buff_Character then
				sm.event.sendToPlayer( self.attacker, "sv_addRobotKill",  { attacker = self.attacker, typeName = "heavyhaybot" } )
			else
				sm.event.sendToPlayer( self.attacker, "sv_addRobotKill",  { attacker = self.attacker, typeName = "haybot" } )
			end
		end
		-- 00Fant
	end
...
function HeavyHayBotUnit.server_onCharacterChangedColor( self, color )
...
function HeavyHayBotUnit.server_onUnitUpdate( self, dt )
...
function HeavyHayBotUnit.sv_e_worldEvent( self, params )
		...
		if params.eventName == "projectileHit" then
			if self.unit.character then
				...
				if distanceToProjectile <= 4.0 then
					-- 00Fant 
					if self.eventTarget == nil and sm.exists( params.attacker ) then
						--print( type( params.attacker ) )
						if type( params.attacker ) == "Character" then
							--print( type( params.attacker ) )
							local attackerCharacter =  params.attacker:getCharacter()
							if sm.exists( attackerCharacter ) then
								self.eventTarget = params.attacker.character
							end
						end
						if type( params.attacker ) == "Shape" then
							--print( type( params.attacker ) )
							if sm.exists( params.attacker ) then
								self.eventTarget = params.attacker
							end
						end
					end
					-- 00Fant
...
function HeavyHayBotUnit.server_onProjectile( self, hitPos, hitTime, hitVelocity, projectileName, attacker, damage )
...
function HeavyHayBotUnit.server_onMelee( self, hitPos, attacker, damage )
	if not sm.exists( self.unit ) or not sm.exists( attacker ) then
		return
	end
...
function HeavyHayBotUnit.server_onExplosion( self, center, destructionLevel )
...
function HeavyHayBotUnit.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal )
...
function HeavyHayBotUnit.server_onCollisionCrush( self )
	if not sm.exists( self.unit ) then
		return
	end
...
function HeavyHayBotUnit.sv_updateCharacterTarget( self )
	if self.unit.character then
		sm.event.sendToCharacter( self.unit.character, "sv_n_updateTarget", { target = self.target } )
	end
end

function HeavyHayBotUnit.sv_addStagger( self, stagger )
 ...
 -- 00Fant "attacker"
function HeavyHayBotUnit.sv_takeDamage( self, damage, impact, hitPos, attacker )
	if self.saved.stats.hp > 0 then
		...
		if self.saved.stats.hp <= 0 then
			...
			if type( attacker ) == "Player" then				
				if self.saved.Buff_Character then
					sm.event.sendToPlayer( attacker, "sv_addRobotKill",  { attacker = attacker, typeName = "heavyhaybot" } )
				else
					sm.event.sendToPlayer( attacker, "sv_addRobotKill",  { attacker = attacker, typeName = "haybot" } )
				end
...
function HeavyHayBotUnit.sv_onDeath( self, impact )
...
function HeavyHayBotUnit.sv_spawnParts( self, impact )
	local character = self.unit:getCharacter()
...
function HeavyHayBotUnit.sv_e_receiveTarget( self, params )
...
function HeavyHayBotUnit.sv_e_allyDamaged( self, params )
...
function HeavyHayBotUnit.sv_e_onEnterWater( self )
	ExtinguishFire( self )
end

function HeavyHayBotUnit.sv_e_onStayWater( self ) end


-- 00Fant
function HeavyHayBotUnit.sv_takeDamage2( self, data )
