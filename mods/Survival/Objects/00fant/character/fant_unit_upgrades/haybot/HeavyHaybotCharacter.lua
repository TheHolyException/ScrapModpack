...
function HeavyHaybotCharacter.server_onCreate( self )
	self:server_onRefresh()
end

function HeavyHaybotCharacter.server_onRefresh( self )
	self.sv = {}
	self.sv.spawnPosition = self.character.worldPosition
	self.sv.spawnWorld = self.character:getWorld()
end

function HeavyHaybotCharacter.client_onCreate( self )
	self.cl = {}
	self.cl.animations = {}
	...
	self.character:addRenderable( "$SURVIVAL_DATA/Objects/00fant/character/fant_unit_upgrades/haybot/haybot_armor_1.rend" )
end

function HeavyHaybotCharacter.client_onDestroy( self )
	--print( "-- HaybotCharacter destroyed --" )
end

function HeavyHaybotCharacter.client_onRefresh( self )
	--print( "-- HaybotCharacter refreshed --" )
	self.cl.headAngle = 0.5
end

function HeavyHaybotCharacter.client_onGraphicsLoaded( self )
...
function HeavyHaybotCharacter.client_onGraphicsUnloaded( self )
	self.graphicsLoaded = false
end

function HeavyHaybotCharacter.cl_initGraphics( self )
...
function HeavyHaybotCharacter.cl_initAnimationSwitch( self )
...
function HeavyHaybotCharacter.client_onUpdate( self, deltaTime )
...
function HeavyHaybotCharacter.client_onEvent( self, event )
	self:cl_handleEvent( event )
end

function HeavyHaybotCharacter.cl_handleEvent( self, event )
...
function HeavyHaybotCharacter.sv_n_updateTarget( self, params )
	self.network:sendToClients( "cl_n_updateTarget", params )
end

function HeavyHaybotCharacter.cl_n_updateTarget( self, params )
	self.cl.target = params.target
end

function HeavyHaybotCharacter.sv_e_unitDebugText( self, text )
	-- No sync cheat
