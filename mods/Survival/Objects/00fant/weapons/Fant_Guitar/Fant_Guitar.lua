...
local renderables = {
	"$SURVIVAL_DATA/Objects/00fant/weapons/Fant_Guitar/Fant_Guitar.rend"
}
...
Fant_Guitar.Damage = 20
Fant_Guitar.Range = 4
Fant_Guitar.Speed = 1.45
Fant_Guitar.SwingStaminaSpend = 3.5


Fant_Guitar.swingCount = 2
Fant_Guitar.mayaFrameDuration = 1.0/30.0
Fant_Guitar.freezeDuration = 0.075

Fant_Guitar.swings = { "sledgehammer_attack1", "sledgehammer_attack2" }
Fant_Guitar.swingFrames = { 4.2 * Fant_Guitar.mayaFrameDuration, 4.2 * Fant_Guitar.mayaFrameDuration }
Fant_Guitar.swingExits = { "sledgehammer_exit1", "sledgehammer_exit2" }

function Fant_Guitar.client_onCreate( self )
	self.isLocal = self.tool:isLocal()
	self:init()
	
end

function Fant_Guitar.client_onRefresh( self )
	self:init()
	self:loadAnimations()
end

function Fant_Guitar.init( self )
...
function Fant_Guitar.loadAnimations( self )
...
function Fant_Guitar.set_currenttool( self, tool )
	self.currentTool = tool
	self.network:sendToClients( "cl_set_currenttool", self.currentTool )
end

function Fant_Guitar.cl_set_currenttool( self, tool )
	self.currentTool = tool
end

function Fant_Guitar.client_onUpdate( self, dt )
...
function Fant_Guitar.updateFreezeFrame( self, state, dt )
...
function Fant_Guitar.server_startEvent( self, params )
...
function Fant_Guitar.client_startLocalEvent( self, params )
	self:client_handleEvent( params )
end

function Fant_Guitar.client_handleEvent( self, params )
...
--function Fant_Guitar.sv_n_toggleTumble( self )
--	local character = self.tool:getOwner().character
--	character:setTumbling( not character:isTumbling() )
--end

function Fant_Guitar.client_onEquippedUpdate( self, primaryState, secondaryState )
	...
	if self.pendingRaycastFlag then
		...
		if time >= frameTime and frameTime ~= 0 then
			...
			if success then
				...
				self.network:sendToServer( "sv_HitSound" )
			end
		end
	end
...
function Fant_Guitar.client_onEquip( self )
...
function Fant_Guitar.client_onUnequip( self )
...
function Fant_Guitar.sv_updateBlocking( self, isBlocking )
...
function Fant_Guitar.GetDamageData( self )
...
function Fant_Guitar.sv_HitSound( self )
	self.network:sendToClients( "cl_HitSound" )
end


function Fant_Guitar.cl_HitSound( self )
	--local EffectName = "ToteBot - Hit"
	--sm.effect.playEffect( EffectName, self.tool:getOwner().character.worldPosition, sm.vec3.new( 0, 0, 0 ), sm.quat.identity() )
	...
	--for i = 1, 2 do
		sm.audio.play( Sounds[ math.random( 1, #Sounds ) ], self.tool:getOwner().character.worldPosition )
	--end
end