...
Fant_Popcorn_Launcher = class()

local renderables = {
	"$SURVIVAL_DATA/Objects/00fant/weapons/Fant_Popcorn_Launcher/fant_popcornlauncher.rend"
}
...
function Fant_Popcorn_Launcher.client_onCreate( self )
	...
	local color = sm.color.new("ff0000" )
	self.tool:setTpColor( color )
	self.tool:setFpColor( color )
end

function Fant_Popcorn_Launcher.client_onRefresh( self )
	self:loadAnimations()
...
function Fant_Popcorn_Launcher.loadAnimations( self )
...
function Fant_Popcorn_Launcher.client_onUpdate( self, dt )
...
function Fant_Popcorn_Launcher.client_onEquip( self, animate )
...
function Fant_Popcorn_Launcher.client_onUnequip( self, animate )
...
function Fant_Popcorn_Launcher.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function Fant_Popcorn_Launcher.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function Fant_Popcorn_Launcher.onAim( self, aiming )
	self.aiming = aiming
...
function Fant_Popcorn_Launcher.onShoot( self, dir )
...
function Fant_Popcorn_Launcher.calculateFirePosition( self )
...
function Fant_Popcorn_Launcher.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function Fant_Popcorn_Launcher.calculateFpMuzzlePos( self )
...
function Fant_Popcorn_Launcher.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if sm.container.canSpend( inventory, obj_consumable_fant_popcorn, 1 ) then
			...
			self:onShoot( dir )
			self.network:sendToServer( "sv_n_onShoot", { dir = dir, pos = self.tool:getTpBonePos( "pejnt_barrel" ) + ( sm.camera.getRight() * -0.05 ) } )
...
function Fant_Popcorn_Launcher.cl_onSecondaryUse( self, state )
...
function Fant_Popcorn_Launcher.client_onEquippedUpdate( self, primaryState, secondaryState )
...
function Fant_Popcorn_Launcher.sv_n_onShoot( self, params )
	local pos = params.pos
	local shootPos = pos +  ( params.dir * 0.1 )
	local popcorn = sm.shape.createPart( obj_consumable_fant_popcorn, shootPos, sm.quat.identity(), true, true )
	if popcorn ~= nil then
		sm.physics.applyImpulse( popcorn, params.dir * 1750 )
	end
	self.network:sendToClients( "cl_n_onShoot", { dir = params.dir, pos = params.pos, popcorn = popcorn } )
	
	sm.container.beginTransaction()
	sm.container.spend( self.tool:getOwner():getInventory(), obj_consumable_fant_popcorn, 1, true )
	sm.container.endTransaction()
end

function Fant_Popcorn_Launcher.cl_n_onShoot( self, params )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( params.dir )
		
		if params.popcorn ~= nil then
			sm.physics.applyImpulse( params.popcorn, params.dir * 1750 )
		end
	end
end
