...
Fant_ElectroHammer.Damage = 15
Fant_ElectroHammer.Range = 3.0
Fant_ElectroHammer.Speed = 1.0
Fant_ElectroHammer.SwingStaminaSpend = 1.5
Fant_ElectroHammer.fuel = 0

Fant_ElectroHammer.swingCount = 2
Fant_ElectroHammer.mayaFrameDuration = 1.0/30.0
Fant_ElectroHammer.freezeDuration = 0.075

Fant_ElectroHammer.swings = { "sledgehammer_attack1", "sledgehammer_attack2" }
Fant_ElectroHammer.swingFrames = { 4.2 * Fant_ElectroHammer.mayaFrameDuration, 4.2 * Fant_ElectroHammer.mayaFrameDuration }
Fant_ElectroHammer.swingExits = { "sledgehammer_exit1", "sledgehammer_exit2" }
...
--	print( Electric_Hammers[ attacker:getId() ].tool )
	...
	if Electric_Hammers[ attacker:getId() ].fuel <= 0 then
		Electric_Hammers[ attacker:getId() ].fuel = 0
		local inventory = attacker:getInventory()
		if inventory ~= nil then
			if sm.container.canSpend( inventory, obj_consumable_battery, 1 ) then
				sm.container.beginTransaction()
				sm.container.spend( inventory, obj_consumable_battery, 1, true )
				if sm.container.endTransaction() then
					Electric_Hammers[ attacker:getId() ].fuel = 5
				end		
			end
		end
	end
	if Electric_Hammers[ attacker:getId() ].fuel > 0 then
		Electric_Hammers[ attacker:getId() ].fuel = Electric_Hammers[ attacker:getId() ].fuel - 1
...
function Fant_ElectroHammer.client_onCreate( self )
	self.isLocal = self.tool:isLocal()
	self:init()
	self.OwnerID = self.tool:getOwner():getId()
end

function Fant_ElectroHammer.server_onCreate( self )
	self.OwnerID = self.tool:getOwner():getId()
end

function Fant_ElectroHammer.client_onRefresh( self )
	self:init()
	self:loadAnimations()
end

function Fant_ElectroHammer.init( self )
...
function Fant_ElectroHammer.loadAnimations( self )
...
function Fant_ElectroHammer.set_currenttool( self, tool )
...
function Fant_ElectroHammer.cl_set_currenttool( self, tool )
	self.currentTool = tool
	Electric_Hammers[ self.tool:getOwner():getId() ].tool = tool
	--print(Electric_Hammers[ self.tool:getOwner():getId() ])
end

function Fant_ElectroHammer.client_onUpdate( self, dt )
...
function Fant_ElectroHammer.refill( self )
	if self.fuel <= 0 then
		self.fuel = 0
		...
		if inventory ~= nil then
			if sm.container.canSpend( inventory, obj_consumable_battery, 1 ) then
				...
				if sm.container.endTransaction() then
					...
					Electric_Hammers[ self.tool:getOwner():getId() ].fuel = self.fuel
				end		
			end
		end
	end
...
function Fant_ElectroHammer.setFuel( self, fuel )
	self.fuel = fuel
	Electric_Hammers[ self.tool:getOwner():getId() ].fuel = self.fuel
end


function Fant_ElectroHammer.updateFreezeFrame( self, state, dt )
...
function Fant_ElectroHammer.server_startEvent( self, params )
...
function Fant_ElectroHammer.client_startLocalEvent( self, params )
	self:client_handleEvent( params )
end

function Fant_ElectroHammer.client_handleEvent( self, params )
...
--function Fant_ElectroHammer.sv_n_toggleTumble( self )
--	local character = self.tool:getOwner().character
--	character:setTumbling( not character:isTumbling() )
--end

function Fant_ElectroHammer.client_onEquippedUpdate( self, primaryState, secondaryState )
	...
	if self.pendingRaycastFlag then
		...
		if time >= frameTime and frameTime ~= 0 then
			...
			if self.tool then	
				if Electric_Hammers[ self.tool:getOwner():getId() ].fuel > 0 then	
					rel_Damage = rel_Damage * 4
				end
			end
...
function Fant_ElectroHammer.client_onEquip( self )
...
function Fant_ElectroHammer.client_onUnequip( self )
...
function Fant_ElectroHammer.server_onDestroy( self )
	Electric_Hammers[ self.OwnerID ].tool = ""
end

function Fant_ElectroHammer.client_onDestroy( self )
	Electric_Hammers[ self.OwnerID ].tool = ""
end

function Fant_ElectroHammer.sv_updateBlocking( self, isBlocking )
...
function Fant_ElectroHammer.GetDamageData( self )
