...
local Damage = 20
local ShootVelocity = 250
local ShootForce = 400

Fant_Scrapgun = class()

local renderables = {
	"$SURVIVAL_DATA/Objects/00fant/weapons/Fant_Scrapgun/Fant_Scrapgun.rend"
}
...
function Fant_Scrapgun.client_onCreate( self )
	self.shootEffect = sm.effect.createEffect( "Farmbot - Hit" )

	local pos = self.tool:getTpBonePos( "pejnt_barrel" )
	local dir = self.tool:getOwner().character:getDirection()
	local effectPos = pos + ( dir * 0.5 ) + ( self.tool:getTpBoneDir( "pejnt_barrel" )  * 0.5 )
...
function Fant_Scrapgun.client_onRefresh( self )
	self:loadAnimations()
...
function Fant_Scrapgun.loadAnimations( self )
...
function Fant_Scrapgun.client_onUpdate( self, dt )
	...
	-- local dir = self.tool:getOwner().character:getDirection() --self.tool:getTpBoneDir( "pejnt_barrel" )

	
	-- effectPos = pos + ( dir * 0.8 ) + ( self.tool:getTpBoneDir( "pejnt_barrel" )  * 0.5 )
	...
	if self.tool:isLocal() and self.tool:isInFirstPersonView() then
		...
		if not self.aiming then
			effectPos = firePos + dir * 0.7 
		else
			effectPos = firePos + dir * 0.8
		end
...
function Fant_Scrapgun.client_onEquip( self, animate )
...
function Fant_Scrapgun.client_onUnequip( self, animate )
...
function Fant_Scrapgun.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function Fant_Scrapgun.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function Fant_Scrapgun.onAim( self, aiming )
	self.aiming = aiming
...
function Fant_Scrapgun.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function Fant_Scrapgun.sv_consumeAmmo( self, data )
	sm.container.beginTransaction()
	sm.container.spend( data.inventory, data.item, data.amount, true )
	sm.container.endTransaction()
	print( "sv_consumeAmmo")
end

function Fant_Scrapgun.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function Fant_Scrapgun.onShoot( self, dir )
...
function Fant_Scrapgun.calculateFirePosition( self )
...
function Fant_Scrapgun.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function Fant_Scrapgun.calculateFpMuzzlePos( self )
...
function Fant_Scrapgun.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		local hasScrew01 = sm.container.canSpend( inventory, obj_decor_screw01, 1 )
		local hasScrew02 = sm.container.canSpend( inventory, obj_decor_screw02, 1 )
		print( hasScrew01, hasScrew02 )
		if hasScrew01 == true or hasScrew02 == true then
			...
			dir = sm.noise.gunSpread( dir, spreadDeg * 15 )
			...
			if owner then
				...
				rel_Damage = math.random( rel_Damage / 2, rel_Damage )
				if hasScrew01 == true then
					sm.projectile.projectileAttack( "fant_screw01", rel_Damage, firePos, dir * ShootVelocity, owner, fakePosition, fakePositionSelf )
					--self.network:sendToServer( "sv_consumeAmmo", { inventory = inventory, item = obj_decor_screw01, amount = 0 } )
				elseif hasScrew02 == true then
					sm.projectile.projectileAttack( "fant_screw02", rel_Damage, firePos, dir * ShootVelocity, owner, fakePosition, fakePositionSelf )
					--self.network:sendToServer( "sv_consumeAmmo", { inventory = inventory, item = obj_decor_screw02, amount = 0 } )
				else
				end
			end
...
function Fant_Scrapgun.cl_onSecondaryUse( self, state )
...
function Fant_Scrapgun.client_onEquippedUpdate( self, primaryState, secondaryState )
