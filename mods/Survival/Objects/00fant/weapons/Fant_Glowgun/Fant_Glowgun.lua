...
local renderables = {
	"$SURVIVAL_DATA/Objects/00fant/weapons/Fant_Glowgun/Fant_Glowgun.rend"
}
...
function Fant_Glowgun.client_onCreate( self )
	self.shootEffect = sm.effect.createEffect( "SpudgunSpinner - SpinnerMuzzel" )
end

function Fant_Glowgun.client_onRefresh( self )
	self:loadAnimations()
end

function Fant_Glowgun.loadAnimations( self )
...
function Fant_Glowgun.client_onUpdate( self, dt )
...
function Fant_Glowgun.client_onEquip( self, animate )
...
function Fant_Glowgun.client_onUnequip( self, animate )
...
function Fant_Glowgun.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function Fant_Glowgun.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function Fant_Glowgun.onAim( self, aiming )
	self.aiming = aiming
...
function Fant_Glowgun.calculateFirePosition( self )
...
function Fant_Glowgun.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function Fant_Glowgun.calculateFpMuzzlePos( self )
...
function Fant_Glowgun.cl_onSecondaryUse( self, state )
...
function Fant_Glowgun.client_onEquippedUpdate( self, primaryState, secondaryState )
...
function Fant_Glowgun.cl_onPrimaryUse( self, state )
	if self.tool:getOwner().character == nil then
		return
	end

	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		if sm.container.canSpend( sm.localPlayer.getInventory(), obj_consumable_glowstick, 1 ) then
			local firstPerson = self.tool:isInFirstPersonView()
			...
			if owner then
				...
				sm.projectile.projectileAttack( "fant_glowstick", rel_Damage, firePos, dir * ShootVelocity, owner, fakePosition, fakePositionSelf )
				--local impulse = dir * -ShootForce
				--sm.physics.applyImpulse( self.tool:getOwner().character, impulse )
...
function Fant_Glowgun.sv_n_onShoot( self, dir )
	--sm.container.beginTransaction()
	--sm.container.spend( self.tool:getOwner():getInventory(), obj_consumable_glowstick, 1, true )
	--if sm.container.endTransaction() then	
		self.network:sendToClients( "cl_n_onShoot", dir )
	--end
end

function Fant_Glowgun.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function Fant_Glowgun.onShoot( self, dir )
	self.tpAnimations.animations.idle.time = 0
