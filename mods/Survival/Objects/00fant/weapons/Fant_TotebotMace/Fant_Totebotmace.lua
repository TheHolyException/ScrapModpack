...
Fant_Totebotmace = class()

local renderables = {
	"$SURVIVAL_DATA/Objects/00fant/weapons/Fant_TotebotMace/Fant_Totebotmace.rend"
}
...
Fant_Totebotmace.Damage = 15
Fant_Totebotmace.Range = 3.1
Fant_Totebotmace.Speed = 1.5
Fant_Totebotmace.SwingStaminaSpend = 2.5


Fant_Totebotmace.swingCount = 2
Fant_Totebotmace.mayaFrameDuration = 1.0/30.0
Fant_Totebotmace.freezeDuration = 0.075

Fant_Totebotmace.swings = { "sledgehammer_attack1", "sledgehammer_attack2" }
Fant_Totebotmace.swingFrames = { 4.2 * Fant_Totebotmace.mayaFrameDuration, 4.2 * Fant_Totebotmace.mayaFrameDuration }
Fant_Totebotmace.swingExits = { "sledgehammer_exit1", "sledgehammer_exit2" }

function Fant_Totebotmace.client_onCreate( self )
	self.isLocal = self.tool:isLocal()
	self:init()
end

function Fant_Totebotmace.client_onRefresh( self )
	self:init()
	self:loadAnimations()
end

function Fant_Totebotmace.init( self )
...
function Fant_Totebotmace.loadAnimations( self )
...
function Fant_Totebotmace.client_onUpdate( self, dt )
...
function Fant_Totebotmace.updateFreezeFrame( self, state, dt )
...
function Fant_Totebotmace.server_startEvent( self, params )
...
function Fant_Totebotmace.client_startLocalEvent( self, params )
	self:client_handleEvent( params )
end

function Fant_Totebotmace.client_handleEvent( self, params )
...
function Fant_Totebotmace.client_onEquippedUpdate( self, primaryState, secondaryState )
	...
	if self.pendingRaycastFlag then
		...
		if time >= frameTime and frameTime ~= 0 then
			...
			if success then
				if result then
					self.network:sendToServer( "sv_hitEffect", {position = result.pointWorld } )
					local character = result:getCharacter()
					if character then
						if unit_totebot_green == character:getCharacterType() then
							local unit = character:getUnit()
							if unit then
								self.network:sendToServer( "sv_performHit", { character = character } )
							end
							rel_Damage = rel_Damage * 3
						end
					end
				end
...
function Fant_Totebotmace.client_onEquip( self )
...
function Fant_Totebotmace.client_onUnequip( self )
...
function Fant_Totebotmace.sv_updateBlocking( self, isBlocking )
...
function Fant_Totebotmace.GetDamageData( self )
...
function Fant_Totebotmace.sv_performHit( self, data )
	if data ~= nil then
		if data.character ~= nil then
			if data.character:getUnit() ~= nil then
				sm.event.sendToUnit( data.character:getUnit(), "sv_Totebothammerhit" ) 
			end
		end
	end
end

function Fant_Totebotmace.sv_hitEffect( self, data )
	self.network:sendToClients( "cl_hitEffect", data )
end

function Fant_Totebotmace.cl_hitEffect( self, data )
	local EffectName = "ToteBot - Hit"
	sm.effect.playEffect( EffectName, data.position, sm.vec3.new( 0, 0, 0 ), sm.quat.identity() )
	--sm.effect.playEffect( EffectName, data.position, sm.vec3.new( 0, 0, 0 ), sm.quat.identity() )
	--sm.effect.playEffect( EffectName, data.position, sm.vec3.new( 0, 0, 0 ), sm.quat.identity() )
end
