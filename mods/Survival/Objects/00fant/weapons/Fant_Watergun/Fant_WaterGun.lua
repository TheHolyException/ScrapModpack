...
local renderables = {
	"$SURVIVAL_DATA/Objects/00fant/weapons/Fant_Watergun/Fant_Watergun.rend"
}
...
function WaterGun.client_onCreate( self )
	self.shootEffect = sm.effect.createEffect( "Mountedwatercanon - Shoot", self.interactable )
end
...
function WaterGun.loadAnimations( self )
...
function WaterGun.client_onUpdate( self, dt )
...
function WaterGun.client_onEquip( self, animate )
...
function WaterGun.client_onUnequip( self, animate )
...
function WaterGun.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function WaterGun.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function WaterGun.onAim( self, aiming )
	self.aiming = aiming
...
function WaterGun.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )

	sm.container.beginTransaction()
	sm.container.spend( self.tool:getOwner():getInventory(), obj_consumable_water, 1, true )
	if sm.container.endTransaction() then	
		self.network:sendToClients( "cl_recoilImpulse", dir )
	end
end

function WaterGun.cl_recoilImpulse( self, dir )
	local impulse = dir * -ShootForce
	sm.physics.applyImpulse( self.tool:getOwner().character, impulse )
end

function WaterGun.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function WaterGun.onShoot( self, dir )
...
function WaterGun.calculateFirePosition( self )
...
function WaterGun.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function WaterGun.calculateFpMuzzlePos( self )
...
function WaterGun.cl_onPrimaryUse( self, state )
	if self.tool:getOwner().character == nil then
		return
	end

	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		if sm.container.canSpend( sm.localPlayer.getInventory(), obj_consumable_water, 1 ) then
			local firstPerson = self.tool:isInFirstPersonView()
			...
			if owner then
				...
				sm.projectile.projectileAttack( "water", rel_Damage, firePos, dir * ShootVelocity, owner, fakePosition, fakePositionSelf )
				--local impulse = dir * -ShootForce
				--sm.physics.applyImpulse( self.tool:getOwner().character, impulse )
			end
...
function WaterGun.cl_onSecondaryUse( self, state )
...
function WaterGun.client_onEquippedUpdate( self, primaryState, secondaryState )
