...
function FarmbotUnit.server_onFixedUpdate( self, dt )
	...
	if updateCrushing( self ) then
		print("'FarmbotUnit' was crushed!", type( self.attacker ))
		self:sv_onDeath()
		-- 00Fant
		if type( self.attacker ) == "Player" then				
			sm.event.sendToPlayer( self.attacker, "sv_addRobotKill",  { attacker = attacker, typeName = "farmbot" } )
		end
		-- 00Fant
	end
...
function FarmbotUnit.server_onExplosion( self, center, destructionLevel )
	...
	self:sv_takeDamage( self.saved.stats.maxhp * ( destructionLevel / 10 ) * 0.4, impact, self.unit:getCharacter().worldPosition )
end
...
function FarmbotUnit.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal  )	
	...
	local collisionDamageMultiplier = 1.0 -- 4x collision damage. 1/4 maxhp to CharacterCollision for fall damage calculation.
...
function FarmbotUnit.sv_takeDamage( self, damage, impact, hitPos, attacker )
	if self.saved.stats.hp > 0 then
		...
		if self.saved.stats.hp <= 0 then
			...
			if type( attacker ) == "Player" then
				...
				sm.event.sendToPlayer( attacker, "sv_addRobotKill",  { attacker = attacker, typeName = "farmbot" } )
			end
			-- 00Fant
		else
			self.storage:save( self.saved )
...
-- 00Fant
function FarmbotUnit.sv_takeDamage2( self, data )
...
function FarmbotUnit.sv_spawnParts( self )
	...
	if isSurvival then
		if math.random( 1, 10 ) == 1 then
			local params = { lootUid = obj_powertools_fant_autominer_farmbot_drill, lootQuantity = 1, epic = false }
