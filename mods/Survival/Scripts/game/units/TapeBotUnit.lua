...
function TapebotUnit.server_onFixedUpdate( self, dt )
	...
	if updateCrushing( self ) then
		print("'TapebotUnit' was crushed!", type( self.attacker ))
		self:sv_onDeath( sm.vec3.new( 0, 0, 0 ) )
		-- 00Fant
		if type( self.attacker ) == "Player" then	
			local botType = "tapebot"
			if self.explosiveProjectile then
				botType = "redtapebot"
			end
			sm.event.sendToPlayer( self.attacker, "sv_addRobotKill",  { attacker = attacker, typeName = botType } )
		end
		-- 00Fant
	end
...
function TapebotUnit.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal )
	...
	if damage > 0 then
		...
		self:sv_takeDamage( damage, collisionNormal, false, self.attacker )
	end
...
function TapebotUnit.sv_takeDamage( self, damage, impact, headHit, attacker )
	if self.saved.stats.hp > 0 then
		...
		if self.saved.stats.hp <= 0 or headHit then
			...
			if type( attacker ) == "Player" then
				...
				sm.event.sendToPlayer( attacker, "sv_addRobotKill",  { attacker = attacker, typeName = botType } )
			end
			-- 00Fant
		else
			self.storage:save( self.saved )
...
function TapebotUnit.sv_spawnParts( self, impact, headDestroyed )
	...
	local headShape
	if self.explosiveProjectile then
		--00Fant obj_robotparts_redtapebothead
...
-- 00Fant
function TapebotUnit.sv_takeDamage2( self, data )
