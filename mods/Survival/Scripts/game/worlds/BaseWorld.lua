...
function BaseWorld.server_onCellLoaded(self, x, y)
    ...
    local stackFn = {
        ...
        [tostring(obj_consumable_inkammo)] = function()
            return randomStackAmount(32, 48, 64)
        end,
...
function BaseWorld.sv_e_onChatCommand(self, params)
    ...
    elseif params[1] == "/pipekit" then
        ...
        sm.container.beginTransaction()
        sm.container.collect(container, obj_pneumatic_pump, 4)
        sm.container.collect(container, obj_pneumatic_pipe_T, 20)
        sm.container.collect(container, obj_pneumatic_pipe_cross, 20)
        sm.container.collect(container, obj_pneumatic_pipe_03, 20)
        sm.container.collect(container, obj_pneumatic_pipe_bend, 20)
        sm.container.endTransaction()
