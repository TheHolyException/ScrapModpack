...
function GrowingOilGeyser.sv_updateTicks( self, ticks )
	if not self.sv.regrown and sm.exists( self.harvestable ) then
		...
		if growFraction >= 1 then
			local respawnOil = sm.harvestable.create( hvs_farmables_oilgeyser, self.harvestable.worldPosition, self.harvestable.worldRotation )
			
			respawnOil:setPublicData( { respawn = true } )
