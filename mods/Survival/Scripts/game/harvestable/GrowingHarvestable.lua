...
function GrowingHarvestable.sv_updateTicks( self, ticks )
	if not self.sv.harvested and sm.exists( self.harvestable ) then
		if self.sv.waterTicks > 0 then
			if FastGrow or CreativeFastGrow then
				self.sv.growTicks = self.sv.growTickTime
			else
...
function GrowingHarvestable.client_onUpdate( self, dt )
	...
	if SoilInfo then
		self:client_ShowText( "#ffffff".. tostring( math.floor( growFraction * 100 ) ) .. "%" .. "\n" .. "#0000ff".. tostring( math.floor( waterFraction * 100 ) ) .. "%" )
	else
		self:client_ShowText( "" )
	end
end
...
function GrowingHarvestable.client_ShowText(self, text )
	...
	if text ~= nil and text ~= "" then
		if self.guiInfo == nil then
			...
			self.guiInfo:setMaxRenderDistance( 10 )
		end
		self.guiInfo:setWorldPosition( self.harvestable.worldPosition + sm.vec3.new( 0, 0, 0 ) )
		self.guiInfo:setText( "Text", text )
	else
