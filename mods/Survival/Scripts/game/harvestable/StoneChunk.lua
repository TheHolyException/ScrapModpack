...
function StoneChunk.sv_onHit( self, damage )
	...
	if self.health > 0 then
		...
		if self.health <= 0 then
			...
			local valid, result = sm.physics.raycast( worldPosition + sm.vec3.new( 0, 0, 100 ), ( worldPosition + sm.vec3.new( 0, 0, -100 ) ), self.shape )
			if valid and result then 
				worldPosition = result.pointWorld + sm.vec3.new( 0, 0, 1 )
			else
				worldPosition = worldPosition + sm.vec3.new( 0, 0, 1 )
			end
...
function StoneChunk.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal )
	if type( other ) == "Shape" and sm.exists( other ) then
		--00Fant start
		if other.shapeUuid == obj_powertools_drill or other.shapeUuid == obj_powertools_fant_drill or other.shapeUuid == obj_powertools_fant_drill_small or other.shapeUuid == obj_powertools_fant_drill_large  or other.shapeUuid == obj_powertools_fant_autominer_farmbot_drill then
		--00Fant end
			...
			if angularVelocity:length() > SPINNER_ANGULAR_THRESHOLD then
				...
				if other.shapeUuid == obj_powertools_fant_autominer_farmbot_drill then
					damage = damage * 2
				end
