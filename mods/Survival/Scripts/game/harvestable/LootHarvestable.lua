...
function LootHarvestable.server_onCreate( self )
	...
	g_lootHarvestables[self.harvestable.id] = { uuid = self.sv.saved.uuid, quantity = self.sv.saved.quantity, epic = self.sv.saved.epic, harvestable = self.harvestable }
	--print( g_lootHarvestables[self.harvestable.id] )
	--00Fant end
end

--00Fant
function LootHarvestable.server_onDestroy( self )
	g_lootHarvestables[self.harvestable.id] = nil 
end
--00Fant
...
function LootHarvestable.client_onClientDataUpdate( self, params )
	...
	elseif params.uuid == obj_outfitpackage_rare then
		...
		self.cl.itemEffect:setPosition( self.harvestable:getPosition() + sm.vec3.new( 0, 0, 0.575 ) )
	elseif params.uuid == obj_outfitpackage_epic or params.uuid == obj_powertools_fant_autominer_farmbot_drill then
