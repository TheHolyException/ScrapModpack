...
local function CreateUnit( tag, node, cell )
	...
	local unit
	if tag == "HAYBOT" then
		local TapeBotSpawn = false
		--print( "TapeBotSpawn" .. tostring( node.position ) )
		if node.position.x > 0 and node.position.y > 0 then
			TapeBotSpawn = true
		end
		if math.random( 0, 100 ) < 15 and TapeBotSpawn then
			unit = sm.unit.createUnit( unit_tapebot, node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400  } )
		else
			if math.random( 0, 100 ) < 75 then
				...
				unit = sm.unit.createUnit( unit_heavy_haybot, node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400  } )
			end
		end
	...
	elseif tag == "TOTEBOT_GREEN" then
		...
		if math.random( 0, 100 ) < 75 then
			...
			unit = sm.unit.createUnit( unit_totebot_red, node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
		end
	...
	elseif tag == "GLOWGORP" then
		...
		if math.random( 0, 100 ) < 75 then
			if math.random( 0, 100 ) < 75 then
				...
				unit = sm.unit.createUnit( unit_fant_gary, node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
			end
		else
			unit = sm.unit.createUnit( unit_totebot_blue, node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
		end
	else
