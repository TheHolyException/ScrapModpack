...
WORLD = WORLD or nil
SoilInfo = SoilInfo or false
FastGrow = FastGrow or false
chunkloaders = chunkloaders or {}
chunksAreLoadedByCmd = false
FantHud = true
AllCheats = false
ShowNameTag = true
showtile = false
fant_clouds_active = true
fant_exp_bar_active = true

function SurvivalGame.sv_FastGrow( self, data )
	FastGrow = data.FastGrow
end

function SurvivalGame.sv_hammer( self, data )
	...
	elseif sledgehammerCount > 0 then
		sm.container.beginTransaction()
		sm.container.spend( inventory, tool_sledgehammer, sledgehammerCount )
		sm.container.endTransaction()
	end
end
...
function SurvivalGame.sv_chunk( self, params )
	...
	if self.sv.saved.chunkloaders == nil then
		self.sv.saved.chunkloaders = {}
	end
	table.insert( self.sv.saved.chunkloaders, params )
end

function SurvivalGame.sv_chunksave( self, params )
	print( "Chunk Loaders Saved" )
	self.storage:save( self.sv.saved )
end

function SurvivalGame.sv_chunkload( self, params )
	if chunksAreLoadedByCmd == true then
		self.network:sendToClients( "cl_chunkMsg" )
		return
	end
	chunksAreLoadedByCmd = true
	print( "Chunk Loaders Loaded" )
	if self.sv.saved.chunkloaders == nil then
		self.sv.saved.chunkloaders = {}
	end
	
	if chunkloaders == nil then
		chunkloaders = {}
	end
	print( chunkloaders )
	for i, k in pairs ( chunkloaders ) do 
		if k ~= nil then
			local p = k:getUnit( )
			--k:destroy()
		end
	...
	for i, k in pairs ( self.sv.saved.chunkloaders ) do 
		if k ~= nil then
			--print( k )
			print( k[3])
			local pos = k[3] + sm.vec3.new( 0, 0, 3 )
			local chunkloader = sm.character.createCharacter( k[1],k[2], pos, k[1].character["id"] ) 
			table.insert( chunkloaders, chunkloader )
		end
	end
end

function SurvivalGame.sv_chunkclear( self, params )
	print( "Chunk Loaders Cleared" )
	self.sv.saved.chunkloaders = {}
	self.storage:save( self.sv.saved )
end

function SurvivalGame.cl_chunkMsg( self ) 
	sm.gui.displayAlertText( "Loading chunks cancelled. Map reload required before loading chunks again to prevent duplicates", 8 )
end
...
function SurvivalGame.sv_teleport( self, data )
	if not sm.exists( self.sv.saved.overworld ) then
		sm.world.loadWorld( self.sv.saved.overworld )
	end
	self.sv.saved.overworld:loadCell( math.floor( data.pos.x/64 ), math.floor( data.pos.y/64 ), data.player )
end

function SurvivalGame.sv_block( self, data )
	FantBlock_sv_block( self, data )
end

function SurvivalGame.cl_block( self, data )
	FantBlock_cl_block( self, data )
end

function SurvivalGame.sv_unblock( self, data )
	FantBlock_sv_unblock( self, data )
end

function SurvivalGame.cl_unblock( self, data )
	FantBlock_cl_unblock( self, data )
end

function SurvivalGame.sv_setPlayer_VS_Player( self, state )
	Player_VS_Player = state
	self.network:sendToClients( "cl_setPlayer_VS_Player", state )
end

function SurvivalGame.cl_setPlayer_VS_Player( self, state )
	Player_VS_Player = state
end

function SurvivalGame.sv_skill( self, params )
	sm.event.sendToPlayer( params.player, "sv_skill", params )
end

function SurvivalGame.sv_setLevel( self, params )
	sm.event.sendToPlayer( params.player, "sv_setLevel", params.level )
end

function SurvivalGame.sv_shape( self, data )
...
function SurvivalGame.bindChatCommands( self )
	...
	sm.game.bindChatCommand( "/showtile", {}, "cl_onChatCommand", "toggle the tile boarder display" )
	sm.game.bindChatCommand( "/hammer", { { "int", "toggle", true } }, "cl_onChatCommand", "Default Hammer Remove" )
	sm.game.bindChatCommand( "/soilinfo", { { "int", "toggle", true } }, "cl_onChatCommand", "Soil Info" )
	...
	sm.game.bindChatCommand( "/skill", {}, "cl_onChatCommand", "skill" )
	sm.game.bindChatCommand( "/setlevel", { { "int", "number", true } }, "cl_onChatCommand", "setlevel" )
	sm.game.bindChatCommand( "/exp", {}, "cl_onChatCommand", "exp" )
...
function SurvivalGame.cl_onChatCommand( self, params )
	...
	elseif params[1] == "/spawn" then
		...
		if rayCastValid then
			...
			elseif params[2] == "redtotebot" or params[2] == "red" or params[2] == "rt" then
				spawnParams.uuid = unit_totebot_red
			elseif params[2] == "bluetotebot" or params[2] == "blue" or params[2] == "bl" then
				spawnParams.uuid = unit_totebot_blue
			elseif params[2] == "haybot" or params[2] == "ha" then
				spawnParams.uuid = unit_haybot
			elseif params[2] == "heavyhaybot" or params[2] == "hv" or params[2] == "heavy"  or params[2] == "he" then
				spawnParams.uuid = unit_heavy_haybot
			...
			elseif params[2] == "baby" then
				spawnParams.uuid = unit_babywoc
			elseif params[2] == "gary" then
				spawnParams.uuid = unit_fant_gary			
	...
	elseif params[1] == "/all" then
		if AllCheats then
			...
			self.network:sendToServer( "sv_switchGodMode" )
			self.network:sendToServer( "sv_setLimitedInventory", true )
			self.network:sendToServer( "sv_setTimeProgress", true )
		else
			AllCheats = true
			self.network:sendToServer( "sv_switchGodMode" )
			self.network:sendToServer( "sv_setLimitedInventory", false )
	...
	elseif params[1] == "/chunksave" then
		self.network:sendToServer( "sv_chunksave", { sm.localPlayer.getPlayer(), sm.localPlayer.getPlayer().character:getWorld() } )
	elseif params[1] == "/chunkload" then
		self.network:sendToServer( "sv_chunkload", { sm.localPlayer.getPlayer(), sm.localPlayer.getPlayer().character:getWorld() } )
	elseif params[1] == "/chunkclear" then
		self.network:sendToServer( "sv_chunkclear", { sm.localPlayer.getPlayer(), sm.localPlayer.getPlayer().character:getWorld() } )
	...
	elseif params[1] == "/fastgrow" then
		if not FastGrow then
			FastGrow = true
		else
			FastGrow = false
		end
		self.network:sendToServer( "sv_FastGrow", { FastGrow = FastGrow } )
	elseif params[1] == "/soilinfo" then
		if not SoilInfo then
			SoilInfo = true
		else
			SoilInfo = false
		end
	elseif params[1] == "/hammer" then
		self.network:sendToServer( "sv_hammer", { player = sm.localPlayer.getPlayer() } )
	...
	elseif params[1] == "/pvp" then
		if Player_VS_Player then
			self.network:sendToServer( "sv_setPlayer_VS_Player", false )	
		else
			self.network:sendToServer( "sv_setPlayer_VS_Player", true )	
		end
	...
	elseif params[1] == "/showtile" then
		if not showtile then
			showtile = true
		else
			showtile = false
		end
		sm.gui.chatMessage( "Show Tiles: " .. tostring( showtile ) )
	...
	elseif params[1] == "/skill" then
		self.network:sendToServer( "sv_skill", { player = sm.localPlayer.getPlayer() } )
	elseif params[1] == "/setlevel" then
		self.network:sendToServer( "sv_setLevel", { player = sm.localPlayer.getPlayer(), level =  params[2] } )
	elseif params[1] == "/exp" then
		if not fant_exp_bar_active then
			fant_exp_bar_active = true
		else
			fant_exp_bar_active = false
		end
		sm.gui.chatMessage( "Show Exp Bar: " .. tostring( fant_exp_bar_active ) )
...
function SurvivalGame.sv_switchGodMode( self )
	...
	self.network:sendToClients( "cl_switchGodMode", g_godMode )
end

function SurvivalGame.cl_switchGodMode( self, state )
	if sm.isHost then
		return
	end
	g_godMode = state
