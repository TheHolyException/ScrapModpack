...
local fant_gary_Renderables = {
	"$SURVIVAL_DATA/Objects/00fant/weapons/Fant_Gary/char_fant_gary.rend"
}
local fant_gary_WormTp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_tp_toolgorp.rend", "$SURVIVAL_DATA/Character/Char_Tools/Char_toolgorp/char_toolgorp_tp.rend" }
local fant_gary_WormFp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_fp_toolgorp.rend", "$SURVIVAL_DATA/Character/Char_Tools/Char_toolgorp/char_toolgorp_fp.rend" }
...
sm.tool.preloadRenderables( fant_gary_Renderables )
sm.tool.preloadRenderables( fant_gary_WormTp )
sm.tool.preloadRenderables( fant_gary_WormFp )
...
function CarryTool.cl_loadAnimations( self, activeUid )
	...
	if isAnyOf( activeUid, harvestItems ) then
		...
		self.blendTime = 0.2
	elseif activeUid == obj_character_worm or activeUid == obj_character_gary then
		self.tpAnimations = createTpAnimations(
			self.tool,
			{
...
function CarryTool.client_onEquip( self, animate, test )
	if self.tool:isLocal() then
...
function CarryTool.cl_updateCarryRenderables( self, activeUid, activeColor )
	...
	elseif activeUid == obj_character_gary then
		carryRenderables = fant_gary_Renderables
		animationRenderablesTp = renderablesWormTp
		animationRenderablesFp = renderablesWormTp
...
function CarryTool.client_onEquippedUpdate( self, primaryState, secondaryState )
	...
	if isHarvest and secondaryState ~= sm.tool.interactState.start then
		...
		if result.type == "body" then
			...
			elseif shape:getShapeUuid() == obj_interactive_fant_resource_collector_5x5x8 then
				shapeContainer = shape.interactable:getContainer( 0 )
			end
		...
		if not collision then
			if sm.container.beginTransaction() then
				...
				if sm.container.endTransaction() then
					sm.unit.createUnit( sm.uuid.new( params.characterShape.characterUuid ), spawnPosition, math.random()*2*math.pi, { isPlaced = true } )
				end
			end
		end
	else
