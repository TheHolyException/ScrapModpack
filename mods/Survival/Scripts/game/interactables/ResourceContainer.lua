...
function ResourceContainer.client_onTinker( self, character, state )
	if state == true then 
...
function ResourceContainer.ModuleManager( self )
	local uuid = nil
	local Range = 1
	for i, m in pairs( ModulesItems ) do
...
function ResourceContainer.server_canErase( self )
...
function ResourceContainer.client_canErase( self )
	local containerModule = self.shape:getInteractable():getContainer(1)
