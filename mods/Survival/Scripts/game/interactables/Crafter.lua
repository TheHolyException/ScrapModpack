...
-- Code changed from MJM and 00Fant
-- ( mostly MJM ) ;)
...
function Crafter.sv_buildPipesAndContainerGraph( self )
	...
	--print( "sv_buildPipesAndContainerGraph" )
...
function Crafter.client_onInteract( self, character, state )
	if state == true then
		self.network:sendToServer( "sv_buildPipesAndContainerGraph" )
		...
		-- Search MOD, if a craftbot, call the search mod functions
		if isAnyOf( self.shape:getShapeUuid(), { obj_craftbot_craftbot1, obj_craftbot_craftbot2, obj_craftbot_craftbot3, obj_craftbot_craftbot4, obj_craftbot_craftbot5, obj_interactive_fant_autocrafter } ) then
			self:cl_mod_search( self.keyboardOutput )
		end
...
-- Search MOD, server check for public message data for search
function Crafter.sv_mod_checkForKeyboard( self )
	for _, parent in pairs(self.interactable:getParents()) do
		-- check if is a scripted part
		if parent:getType() == "scripted" then
			-- check if has public data with a "message" value
			local parentData = parent:getPublicData()
			if parentData and parentData.name and parentData.name ~= "" then --TODO, "name" should be whatever keyboard calls the message
				self.network:sendToClients("cl_mod_search", parentData.name) --TODO, "name" should be whatever keyboard calls the message
				return
			end
		end
	end
	self.network:sendToClients("cl_mod_search", nil) -- this nil return will trigger reset of recipes when the message is empty
end

-- Search MOD, perform search
function Crafter.cl_mod_search( self, searchStr )
	if searchStr == nil or searchStr == "" then
		-- if there is no search, check if there was previously one
		if self.previousSearch then
			self:cl_updateRecipeGrid() -- reset recipe grid to normal
			self.previousSearch = false -- clear flag that a search was done
		end
		return -- return since there's no search to do
	end
	self.previousSearch = true -- set flag that a search is being done
	
	-- Create a table of item names by uuid (needed to look up name by ID, so survival_items.lua doesn't work here)
	local namesTable = {}
	local dataDescPath = "$GAME_DATA/Gui/Language/English/InventoryItemDescriptions.json"
	local survivalDescPath = "$SURVIVAL_DATA/Gui/Language/English/inventoryDescriptions.json"
	local inventoryDescPaths = {dataDescPath, survivalDescPath}
	for _, path in pairs(inventoryDescPaths) do
		local json = sm.json.open(path)
		for id, info in pairs(json) do
			namesTable[id] = { keywords = {} } -- kewords and title will all be added in keywords for search
			if info.keywords then -- if description has keywords, set as the value
				namesTable[id].keywords = info.keywords
			end
			if info.title then -- if description has title, add it to the keywords
				table.insert(namesTable[id].keywords, info.title)
			end
		end
	end
	
	-- Search the recipes file for matches, and set them in the first positions
	local recipesPath = "$SURVIVAL_DATA/CraftingRecipes/craftbot.json"
	local recipesJson = sm.json.open(recipesPath)
	local resultRecipes = {}
	if recipesJson == nil then
		--print("ERROR: recipesJson is nil")
	else
		--print("Searching recipes for keyword["..searchStr.."]")
		for _, recipe in pairs(recipesJson) do
			local id = recipe.itemId
			if id == nil then
				--print("WARN: id is nil, skipping")
			else
				--print("ID: "..id) --debug
				local info = namesTable[id]
				if info == nil then
					--print("WARN: no description info, skipping")
				else
					--print("...has info") --debug
					local keywords  = info.keywords
					if keywords == nil then
						--print("WARN: no kewords or title, skipping")
					else
						--print("Has "..#keywords.." keywords") --debug
						-- keywords already had title added to its list, so both are checked
						for _, keyword in pairs(keywords) do
							--print("Check keyword: "..keyword) --debug
							if string.match(string.upper(keyword), string.upper(searchStr)) then
								--print("Match!") --debug
								table.insert( resultRecipes, recipe )
								break
							end
						end
					end
		...
		-- save results to a file and load back into craftbot, because stupid API limitations
		local searchResultsPath = "$SURVIVAL_DATA/CraftingRecipes/craftbot_search.json"
		sm.json.save( resultRecipes, searchResultsPath )
		self.cl.guiInterface:clearGrid( "RecipeGrid" )
		self.cl.guiInterface:addGridItemsFromFile( "RecipeGrid", searchResultsPath, { locked = false } )
	end
...
function Crafter.cl_onUpgrade( self, buttonName )
	...
	if self.shape:getShapeUuid() ~= obj_craftbot_craftbot5 then
		self.network:sendToServer( "sv_n_upgrade" )
	end
end
...
function Craftbot.client_canInteract_OFF( self, character )
	...
	if not self.crafter.needsPower or ( parent and parent.active ) then
		...
		sm.gui.setInteractionText( "", keyBindingText2, "Search" )
	else
		sm.gui.setCenterIcon( "Hit" )
		sm.gui.setInteractionText( "#{INFO_REQUIRES_POWER}" )
		return false
...
function Craftbot.client_onTinker_OFF( self, character, state )
	if state then
		if character == sm.localPlayer.getPlayer().character then
			...
			self.keyboardGui:setText( "output_text", self.keyboardOutput )
		end
	end
end

function Craftbot.cl_onKeyboard( self, key )
	--print( "cl_onKeyboard", key )
	if key ~= nil then
		if self.keyboardOutput == nil then
			self.keyboardOutput = "Search Word"
		end
		...
		if input == "DEL" then
			isCmd = true
			--print( "keyboard DEL" )
			if self.keyboardOutput:len() == 1 then
				self.keyboardOutput = ""
			else
				local s = string.reverse( self.keyboardOutput )
				s = string.sub( s, -( self.keyboardOutput:len() - 1 ) )
			
				self.keyboardOutput = string.reverse( s )
			end
		end
		if input == "SPACE" then
			isCmd = true
			--print( "keyboard SPACE" )
			self.keyboardOutput = self.keyboardOutput .. " "
		end
		if input == "COMMA" then
			isCmd = true
			--print( "keyboard COMMA" )
			self.keyboardOutput = self.keyboardOutput .. ","
		end
		if input == "DOT" then
			isCmd = true
		...
		if input == "MINUS" then
			isCmd = true
			--print( "keyboard MINUS" )
			self.keyboardOutput = self.keyboardOutput .. "-"
		end
		if input == "UNDERSCORE" then
			isCmd = true
			--print( "keyboard UNDERSCORE" )
			self.keyboardOutput = self.keyboardOutput .. "_"
		end
		
		if not isCmd then
			if self.keyboardOutput == "Search Word" then
				self.keyboardOutput = ""
			end
			self.keyboardOutput = self.keyboardOutput .. input
		end
...
function Craftbot.cl_onClose( self )
	self.keyboardGui:close()
end