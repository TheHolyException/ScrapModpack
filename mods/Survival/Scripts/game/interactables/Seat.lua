...
function Seat.server_onFixedUpdate( self, dt )
	self.interactable:setActive( self.interactable:getSeatCharacter() ~= nil )
	
	if self.feedDelay <= 0 then
		self.feedDelay = 1
		local children = self.shape:getInteractable():getChildren()
		local childInteractable = nil
		if #children > 0 then
			for _, child in ipairs( children ) do					
				if child then
					if child.shape ~= nil then
						if child.shape:getShapeUuid() == obj_interactive_fridge then
							childInteractable = child.shape:getInteractable()
							break
						end
		...
		if childInteractable ~= nil then
			local seatedCharacter = self.shape:getInteractable():getSeatCharacter()
			if seatedCharacter then
				local ID = seatedCharacter:getPlayer():getId()
				if g_Players ~= nil then
					if g_Players[ID] ~= nil then
						if g_Players[ID].water ~= nil and g_Players[ID].food ~= nil then
							if g_Players[ID].water < self.FeedLimited or g_Players[ID].food < self.FeedLimited then
								local foodcontainer = childInteractable:getContainer( 0 )
								if foodcontainer ~= nil then
									if true then
										for slot = 0, foodcontainer:getSize() - 1 do
											local FoodItem = foodcontainer:getItem( slot )											
											if FoodItem then									
												if FoodItem.quantity >= 1 then
													local edible = sm.item.getEdible( FoodItem.uuid )
													if edible ~= nil then
														sm.container.beginTransaction()
														sm.container.setItem( foodcontainer, slot, FoodItem.uuid, FoodItem.quantity - 1 )
														if sm.container.endTransaction() then
															sm.event.sendToPlayer( seatedCharacter:getPlayer(), "sv_e_eat", { hpGain = edible.hpGain, foodGain = edible.foodGain, waterGain = edible.waterGain } )
															break
														end
													end
												end
		...
		self.feedDelay = self.feedDelay - dt
	end
end
