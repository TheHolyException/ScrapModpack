...
function Wardrobe.cl_onClose( self )
	if self.cl.user then
		...
		-- 00Fant
		self.network:sendToServer( "sv_refreshCustomCloths", self.cl.user:getPlayer() )
		-- 00Fant
...
function Wardrobe.client_onUpdate( self, dt )
	...
	if self.cl.user and self.cl.user == character then
		...
		-- 00Fant
		if fant_cloth_refresh_timer <= 0 then
			fant_cloth_refresh_timer = 0.25
			self.network:sendToServer( "sv_refreshCustomCloths", character:getPlayer() )
		else
			fant_cloth_refresh_timer = fant_cloth_refresh_timer - dt
		end
		-- 00Fant
...
function Wardrobe.sv_refreshCustomCloths( self, player )
	sm.event.sendToPlayer( player, "sv_extern_fant_cloth_reset" )
end